...
cc.Class({
	...
	Init: function (uiPos, dataPos, point, isLeft = -1, bResetPoint = true, useBottomChat = false, lb_info = null, isShowOK = false) {
		this.uiPos = uiPos;
		...
		this.lb_name = this.GetWndComponent("touxiang/lb_name", cc.Label);
		this.lb_jifen = this.GetWndComponent("touxiang/sp_info/lb_jifen", cc.Label);
		this.zhuangnum = this.GetWndComponent("touxiang/zhuangjia/img_er/zhuangnum", cc.Label);
		...
		app[app.subGameName + "Client"].RegEvent("Head_PosReady", this.Event_PosReady, this);
		...
		this.left = {x: -99, y: 0};
		this.right = {x: 99, y: 0};
		this.node.name = app.subGameName + '_UIPublicHead' + this.uiPos;
		if (!isShowOK) {
	...
	HideAllChild: function () {
		...
		this.zhuangnum.node.parent.active = false;
	...
	ShowChatContent: function (content) {
		...
		// this.time = new Date().getTime() + this.PropertyInfo["HeadShowMesssageTime"];
	...
	ShowBtnOut: function () {
		...
		if (this.playerInfo.pid == this.ownerID) {
			this.btn_out.active = false;
		} else {
	...
	ShowPlayerBaseInfo: function (room) {
		...
		this.HideBtnOut();
		if (this.playerInfo.pid == ownerID) {
			iscreateID = true;
		} else {
			this.ShowBtnOut();
		}
	...
	ShowZhuangNum: function (zhuangNum = "Not") {
		if (zhuangNum == "Not") {
			this.zhuangnum.node.parent.active = false;
			return;
		}
		this.zhuangnum.node.parent.active = true;
		this.zhuangnum.string = zhuangNum;
	},
