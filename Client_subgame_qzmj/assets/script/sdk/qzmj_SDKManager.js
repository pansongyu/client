...
var qzmj_SDKManager = app.BaseClass.extend({
	...
	ShareCN:function(title,description,urlStr){
		if(cc.sys.isNative){
			let appTiShi=false;
	        ...
	        if(appTiShi==true){
	        	app[app.subGameName+"_SysNotifyManager"]().ShowSysMsg("您当前游戏版本过低，需要重新下载才能分享吹牛");
	            return;
	        }
    	}

		let argList = [{"Name":"title","Value":title},{"Name":"description","Value":description},{"Name":"urlStr","Value":urlStr}];
		app[app.subGameName + "_NativeManager"]().CallToNative("OnCNShare", argList);
	},
...
var g_qzmj_SDKManager = null;

/**
 * 绑定模块外部方法
 */
exports.GetModel = function(){
    if(!g_qzmj_SDKManager){
        g_qzmj_SDKManager = new qzmj_SDKManager();
    }
    return g_qzmj_SDKManager;
}