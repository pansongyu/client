...
var qzmj_SceneManager = app.BaseClass.extend({
	...
	LoadScene: function (sceneName, mapID = 0, isReLoad = false) {
		...
		if (sceneName != 'loginScene') {
			...
			let that = this;
			cc.loader.loadRes("ui/qzmj_UIWaitForm", function (err, prefab) {
	...
	RealGamePrefab: function (GamePrefab) {
		...
		if (is3DShow == 2 && GamePrefab == "game/" + app.subGameName.toUpperCase() + "/ui/UI" + app.subGameName.toUpperCase() + "Play") {
			//2D场景需要切换
			GamePrefab = "game/" + app.subGameName.toUpperCase() + "/ui/UI" + app.subGameName.toUpperCase() + "Play";
		}
...
var g_qzmj_SceneManager = null;

/**
 * 绑定模块外部方法
 */
exports.GetModel = function () {
	if (!g_qzmj_SceneManager) {
		g_qzmj_SceneManager = new qzmj_SceneManager();
	}
	return g_qzmj_SceneManager;
}
