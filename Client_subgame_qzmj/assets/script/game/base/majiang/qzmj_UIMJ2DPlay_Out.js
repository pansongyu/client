...
cc.Class({
	...
	ReSetPaiChildCount:function(){
		/*if(this.RoomMgr.GetEnterRoom()){
			let playerCount=this.RoomMgr.GetEnterRoom().GetRoomPosMgr().GetRoomPlayerCount();
			if(playerCount==2){
				this.PaiChildCount = 15;
			}else{
				this.PaiChildCount = 9;
			}
		}*/
	},
	HideAllChild:function(){
		this.ReSetPaiChildCount();
	...
	ShowOutCard2:function(outcard,jin1=0,jin2=0,imageString){
		let outCardList = this.OutCard(outcard);
		let count = outCardList.length;
		let layOutIndex = 1;
		let paiIndex = 1;
		let forNum = 0;
