...
cc.Class({
	...
	ShowAllOutEatCard: function () {
		...
		else if (nodeParentName.indexOf("Card04") >= 0) {
		...
		for (let index = 0; index < count; index++) {
			...
			for (let cardIndex = 0; cardIndex < cardCount; cardIndex++) {
				...
				this.ShowImage(childNode, imageString, cardID);
				//显示头搭二搭被搭
				// if(cardIndex<3 || opType==this.ShareDefine.OpType_JieGang){
				// 	this.ShowDa(childNode.getChildByName('da'),cardID,daList);
				// }else{
				// 	//杠第四张要另外判断
				// 	this.ShowDa2(childNode.getChildByName('da'),cardID,daList);
				// }
	...
	ShowDa2: function (childNode, cardID, daList) {
		let touda = daList[0];
		let erda = daList[1];
		let beida = daList[2];
		let daId = 0;
	...
	ShowDa: function (childNode, cardID, daList) {
		...
		if (beida.length > 0) {
			for (let i = 0; i < beida.length; i++) {
				if (cardType == Math.floor(beida[i] / 100)) {
					daId = 3;
					break;
				}
			}
		...
		if (erda.length > 0) {
			for (let i = 0; i < erda.length; i++) {
				if (cardType == Math.floor(erda[i] / 100)) {
					daId = 2;
					break;
				}
			}
		...
		if (touda.length > 0) {
			for (let i = 0; i < touda.length; i++) {
				if (cardType == Math.floor(touda[i] / 100)) {
					daId = 1;
					break;
				}
			}
	...
	ShowImage: function (childNode, imageString, cardID) {
		...
		if (cardID) {
			...
			} else if (imageString == "OutCardYF_Self_") {
				imageName = "OutCardYF_Self_50";
			} else if (imageString == "OutCardYF_Up_") {
				imageName = "OutCardYF_Up_50";
			} else if (imageString == "OutCardYF_Down_") {
				imageName = "OutCardYF_Down_50";
			}
		}
		let imageInfo = this.IntegrateImage[imageName];
		if (!imageInfo) {
