...
cc.Class({
	...
	ShowHandCard: function (room) {
		...
		if (this.btn_in.getChildByName("card").getChildByName('jin')) {
			this.btn_in.getChildByName("card").getChildByName('jin').active = false;
		}
		if (handCard <= 0) {
	...
	ShowAllPlayerCard: function (room) {
		...
		for (let index = 0; index < count; index++) {
			...
			if (cardNode.getChildByName('jin')) {
				cardNode.getChildByName('jin').active = false;
			}
	...
	ShowTingPai: function (cardMap) {
		...
		if (cardMap.length > 0) {
			this.FormManager.ShowForm("game/base/ui/majiang/" + app.subGameName + "_UIMJWBHuPai", cardMap, huCardTypeInfo);
		}
	},
	//显示本家出牌动作,提示
