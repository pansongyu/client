 ...
 */
	...
	ChooseCardNode: function (clickNode) {
		...
		let buChuList = this.GetBuChuList();

		let clickCardID = clickNode.getChildByName("card").getComponent(app.subGameName+"_UIMJCard01_Card").GetCardID();

		if (buChuList.InArray(Math.floor(clickCardID / 100))) {
		...
		if(this.chooseNode){
			...
			if(this.chooseNode == clickNode){
				...
				this.canTouch = false;
				//this.kehu.active=false;
				this.FormManager.CloseForm("game/base/ui/majiang/"+app.subGameName+"_UIMJHuPai");
				let self = this;
				setTimeout(function(){self.canTouch = true;},2000);//防止刚打出去立马又touch了
			}
			else{
				this.chooseNode.y= this.InitBtnCardPosY;
				this.chooseNode = clickNode;
				...
				let clickCardType=Math.floor(clickCardID/100);
				if(GetTingCardMap[clickCardType]){
					//显示听牌数组
					this.ShowTingPai(GetTingCardMap[clickCardType]);
				}else{
					this.FormManager.CloseForm("game/base/ui/majiang/"+app.subGameName+"_UIMJHuPai");
					//this.kehu.active=false;
				}
