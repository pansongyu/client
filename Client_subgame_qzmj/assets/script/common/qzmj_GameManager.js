...
var qzmj_GameManager = app.BaseClass.extend({
	...
	OnPack_MqResponseBo:function(event){
		...
		if(code==0){
            ...
            //关闭游戏的UI
            app[app.subGameName + "_FormManager"]().CloseForm("game/" + app.subGameName.toUpperCase() + "/ui/UI" + app.subGameName.toUpperCase() + "2DPlay");
            app[app.subGameName + "_FormManager"]().CloseForm("game/" + app.subGameName.toUpperCase() + "/ui/UI" + app.subGameName.toUpperCase() + "Play");
            app[app.subGameName + "_FormManager"]().CloseForm("game/" + app.subGameName.toUpperCase() + "/ui/UI" + app.subGameName.toUpperCase() + "WBPlay");
            
            app[app.subGameName + "_FormManager"]().CloseForm("game/base/ui/majiang/" + app.subGameName + "_UIMJResultDetail");
            //关闭游戏的UI
	...
	OnPack_PosLeave: function (serverPack) {
		...
		if (!serverPack["beKick"] && selfPos == pos) {
	...
	OnPack_XiPai: function (serverPack) {
		...
		if ('PK' == cType) {
			app[app.subGameName + "_FormManager"]().ShowForm('UIXiPai');
		}
		else {
			app[app.subGameName + "_FormManager"]().ShowForm("game/" + app.subGameName.toUpperCase() + "/base/" + app.subGameName + "_UIMJXiPai");
		}
	},
	...
	SendXiPai: function (roomID) {
		this.NetManager.SendPack(app.subGameName + ".C" + app.subGameName.toUpperCase() + "XiPai", {"roomID": roomID});
	},
});


var g_qzmj_GameManager = null;

/**
 * 绑定模块外部方法
 */
exports.GetModel = function () {
	if (!g_qzmj_GameManager) {
		g_qzmj_GameManager = new qzmj_GameManager();
	}
	return g_qzmj_GameManager;
}