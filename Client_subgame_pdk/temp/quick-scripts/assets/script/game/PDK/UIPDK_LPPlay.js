(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/script/game/PDK/UIPDK_LPPlay.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, 'typdk8df-2d44-49fd-bd1c-e09e7ba71841', 'UIPDK_LPPlay', __filename);
// script/game/PDK/UIPDK_LPPlay.js
...
cc.Class({
    ...
    AddCardPrefab: function AddCardPrefab() {
        ...
        for (var _i = 0; _i < this.Define.MaxPlayer; _i++) {
            ...
            var typeNode = cc.find("outCardList" + _i + "_cardType", this.node);
    ...
    HideAllOutCard: function HideAllOutCard() {
        for (var i = 0; i < this.Define.MaxPlayer; i++) {
            ...
            var typeNode = cc.find("outCardList" + i + "_cardType", this.node);
            typeNode.getComponent(cc.Sprite).spriteFrame = null;
    ...
    fk3MoveOver: function fk3MoveOver() {
        if (!this.isFk3()) return;
        var dataPos = this.RoomSet.GetRoomSetProperty("opPos");
        var uiPos = this.RoomPosMgr.GetUIPosByDataPos(dataPos);
    ...
    fk3GoWhere: function fk3GoWhere() {
        if (!this.isFk3()) return;
        this.fk3.getComponent(cc.Animation).stop();
        var dataPos = this.RoomSet.GetRoomSetProperty("opPos");
        var uiPos = this.RoomPosMgr.GetUIPosByDataPos(dataPos);
        var playerNode = cc.find("sp_seat0" + uiPos, this.node);
        var playerHead = cc.find('head', playerNode);
        var movePos = cc.v2(playerNode.x + playerHead.x, playerNode.y + playerHead.y);
        var moveAction = cc.moveTo(0.15, movePos);
        var scaleAction = cc.scaleTo(0.15, 0.2);
        var spawnAction = cc.spawn(moveAction, scaleAction);
        var self = this;
        var action = cc.sequence(spawnAction, cc.callFunc(self.fk3MoveOver, self));
    ...
    AddHead: function AddHead() {
        ...
        for (var idx = 0; idx < posList.length; idx++) {
            ...
            if (!node.getChildByName("LPUIPublicHead" + uiPos)) {
                ...
                node.addChild(head);
                var headScript = head.getComponent(app.subGameName + "_LPUIPublicHead");
                headScript.Init(uiPos, posList[idx].pos, cc.v2(0, 0), this.Left[uiPos]);
            } else {
                var headNode = node.getChildByName("LPUIPublicHead" + uiPos);
                var _headScript = headNode.getComponent(app.subGameName + "_LPUIPublicHead");
    ...
    ClearHead: function ClearHead() {
        for (var idx = 0; idx < this.Define.MaxPlayer; idx++) {
            var path = "sp_seat0" + idx + "/head" + "/LPUIPublicHead" + idx;
            var node = this.GetWndNode(path);
            if (node) {
                var headScript = node.getComponent(app.subGameName + "_LPUIPublicHead");
                headScript.OnClose();
            }
            if (idx > 0) {
                var seat = cc.find("sp_seat0" + idx, this.node);
                ...
                var head = cc.find("sp_seat0" + idx + "/head", this.node);
    ...
    OnShow: function OnShow() {
        ...
        this.schedule(function () {
            var dateE = new Date();
            var dian = cc.sys.getBatteryLevel();
            var hours = dateE.getHours();
            var minutes = dateE.getMinutes();
            _this2.lb_time.string = (hours < 10 ? '0' + hours : hours) + ":" + (minutes < 10 ? '0' + minutes : minutes);
            _this2.lb_dianLiang.string = Math.floor(dian * 100) + '%';
            _this2.pro_dianLiang.progress = dian;
        }, 1);
    ...
    //老版皮肤
    onShowGame1: function onShowGame1() {
        this.labeiWanfa.node.color = cc.color(40, 82, 109);
    },

    //新增皮肤
    onShowGame2: function onShowGame2() {},

    ChangeBg: function ChangeBg(GameBgID) {
    ...
    isZhaDanKeCai: function isZhaDanKeCai() {
    ...
    HideAllZhunBeiLabel: function HideAllZhunBeiLabel() {
        ...
        for (var i = 0; i < playerAllList.length; i++) {
            ...
            var path = "sp_seat0" + uiPos + "/head" + "/LPUIPublicHead" + uiPos;
            var headNode = this.GetWndNode(path);
            if (!headNode || typeof headNode == "undefined") continue;
            var headScript = headNode.getComponent(app.subGameName + "_LPUIPublicHead");
    ...
    HideAllMessage: function HideAllMessage() {
        if (this.messageLayer == null) {
            this.messageLayer = cc.find('messageLayer', this.node);
        }
        for (var i = 0; i < this.messageLayer.childrenCount; i++) {
            this.messageLayer.children[i].active = false;
        }
    },

    showHideMessageOfChildrenName: function showHideMessageOfChildrenName(childrenName, isShow) {
        if (this.messageLayer == null) {
            this.messageLayer = cc.find('messageLayer', this.node);
        }
    ...
    hideImg_ts_db: function hideImg_ts_db() {
...
cc._RF.pop();
        ...
        //# sourceMappingURL=UIPDK_LPPlay.js.map
