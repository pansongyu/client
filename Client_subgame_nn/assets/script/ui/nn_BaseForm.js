...
var nn_BaseForm = cc.Class({
	...
	ShowForm: function (argList) {
		...
		if (!parent) {
			console.error("ShowForm not find uiLayer", parent, this._dataInfo);
			return
		}
	...
	CreateChildForm: function (childComponentName, childPrefab, userData = 0, parentNode = null, pos = null, zorderLv = 0, argument = null, isShowForm = true) {
		...
		if (!childFormComponent) {
			...
			console.warn("CreateChildForm not find:%s addComponent", childComponentName);
	...
	SetNodeImageByFilePath: function (node, filePath, isHideFirst = false) {
		...
		return app[app.subGameName + "_ControlManager"]().CreateLoadPromise(filePath, cc.SpriteFrame)
			.then(function (spriteFrame) {
				if (!spriteFrame) {
					// console.error("SetNodeImageByFilePath load spriteFrame fail filePath : ", filePath);
					return false
				}
	...
	SetWndImageByFilePath: function (wndPath, filePath, fileName = "", isHideFirst = false) {
		...
		return app[app.subGameName + "_ControlManager"]().CreateLoadPromise(filePath, resType)
			.then(function (spriteFrame) {
				...
				if (!spriteFrame) {
					// console.error("SetWndImageByFilePath(%s,%s,%s) load spriteFrame fail", wndPath, filePath, fileName);
					return false
				}
			...
			.catch(function (error) {
				// console.error("SetWndImageByFilePath(%s,%s,%s) error:%s", wndPath, filePath, fileName, error.stack);
			})
	...
	AddWndEffect: function (wndPath, effectName, effectRes, repeatCount = 0, userData = 0, offPosList = null, posType = null) {
		...
		this.EffectManager.CreateEffectByEffectRes(effectName, effectRes, userData)
			...
			.catch(function (error) {
				console.error("CreateFormEffect(%s,%s,%s):%s", effectName, effectRes, userData, error.stack);
			})
	...
	Click_btn_weixin: function (isShareScreen = false, share = "") {
		...
		if (share == "WX") {
			...
			this.FormManager.ShowForm(app.subGameName + '_UIRoomCopy', roomKey, title, gamedesc, weChatAppShareUrl, isShareScreen);
		}
	},
	...
	WanFa: function (cfg = null, gameName = null) {
