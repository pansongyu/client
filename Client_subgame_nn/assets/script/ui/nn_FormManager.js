...
var nn_FormManager = app.BaseClass.extend({
	...
	CloseForm: function (formPath, isDestory = false) {
		...
		let formName = this.formPath2formName(formPath);
		if (!this.loadFormDict.hasOwnProperty(formPath)) {
			console.log("CloseForm(%s) not load", formPath);
			return
		}
		let formNode = this.loadFormDict[formPath];
		if (!formNode) {
	...
	GetFormComponentByFormName: function (formPath) {
		...
		let formName = this.formPath2formName(formPath);
		if (!this.loadFormDict.hasOwnProperty(formPath)) {
			console.log("GetFormComponentByFormName(%s) not load", formPath);
			return
		}
	...
	IsFormShow: function (formPath) {
		...
		let formName = this.formPath2formName(formPath);
		if (!this.loadFormDict.hasOwnProperty(formPath)) {
			console.log("IsFormShow(%s) not load", formPath);
			return false
		}
	...
	GetFormWndShowInfo: function (formPath) {
		...
		if (!this.formWndShowInfo.hasOwnProperty(formPath)) {
			console.log("GetFormWndShowInfo not find:%s", formPath);
			return {}
		}
	...
	OnBeforeExitScene: function (sceneName, isDestory = true) {
		console.log("OnBeforeExitScene(%s)", sceneName);
		//关闭所有界面
	...
	LoadSceneDefaultForm: function (sceneName) {
		...
		console.log("LoadSceneDefaultForm(%s)", sceneName);
		...
		let BaseForm=[];
		// let BaseForm = [app.subGameName + '_UIMark', app.subGameName + '_UIVoice', app.subGameName + '_UIMore'];
	...
	OnSwithSceneEnd: function (sceneName) {
		...
		console.log("OnSwithSceneEnd(%s)", sceneName);
...
exports.GetModel = function () {
	if (!g_nn_FormManager) {
		g_nn_FormManager = new nn_FormManager();
	}
	return g_nn_FormManager;
}
