...
var nn_GameManager = app.BaseClass.extend({
	...
	OnPack_PosUpdate: function (serverPack) {
		console.log("服务器发来更新座位信息 OnPack_PosUpdate", serverPack);
		let roomID = serverPack["roomID"];
		let pos = serverPack["pos"];
		...
		if (this.NowRoom().GetRoomPosMgr().OnPosUpdate(pos, posInfo)) {
			...
			app[app.subGameName + "Client"].OnEvent(app.subGameName.toUpperCase() + "_AllPosUpdate", serverPack);
		} else {
	...
	OnPack_XiPai: function (serverPack) {
		...
		if ('PK' == cType) {
			app[app.subGameName + "_FormManager"]().ShowForm(app.subGameName + "_UIXiPai");
		} else {
	...
	SendReady: function (roomID, posIndex) {
		this.NetManager.SendPack(app.subGameName + ".C" + app.subGameName.toUpperCase() + "ReadyRoom", {
			...
			console.log("位置发送准备状态", success);
		}, function (error) {
			console.log("位置发送准备状态", error, roomID, posIndex);
		});
	},
...
exports.GetModel = function () {
	if (!g_nn_GameManager) {
		g_nn_GameManager = new nn_GameManager();
	}
	return g_nn_GameManager;
}