...
var nn_DownLoadMgr = app.BaseClass.extend({
    ...
    OnDownLoadEvent:function(serverPack){
        if(serverPack["state"] == nn_DownLoadMgr.DwonLoadState_Finish){
            this.OnDownFileFinish(serverPack);
        }else if(serverPack["state"] == nn_DownLoadMgr.DwonLoadState_Error){
            this.OnDownFileError(serverPack);
        }else if (serverPack["state"] == nn_DownLoadMgr.DwonLoadState_Progress){
    ...
    GetDownLoadStateError:function(){
        return nn_DownLoadMgr.DwonLoadState_Error;
    },
    //获取成功标识
    GetDownLoadStateFinish:function(){
        return nn_DownLoadMgr.DwonLoadState_Finish;
    },
    //获取进度标识
    GetDownLoadStateProgress:function(){
        return nn_DownLoadMgr.DwonLoadState_Progress;
    },
});


DownLoadstate.apply(nn_DownLoadMgr, []);

var g_nn_DownLoadMgr = null;
  /**
  ...
  */
exports.GetModel = function(){
    if(!g_nn_DownLoadMgr){
        g_nn_DownLoadMgr = new nn_DownLoadMgr();
    }
    return g_nn_DownLoadMgr;
}