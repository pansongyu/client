...
var nn_AudioManager = app.BaseClass.extend({
    ...
    sendRecordFile:function(){
        ...
        httpRequest.onerror = function(){
            console.error("sendRecordFile httpRequest.error:%s", url);
            app[app.subGameName + "_SysNotifyManager"]().ShowSysMsg('MSG_AUDIO_SEND_FAIL');
        };
        
        httpRequest.onreadystatechange = function(){
            console.log("onreadystatechange httpRequest.status:%s, httpRequest.responseText:%s",httpRequest.status, httpRequest.responseText);
            //执行成功
            if (httpRequest.status == 200){
                
                var receivePack = JSON.parse(httpRequest.responseText);
                console.log("sendRecordFile receivePack[status]:%s,receivePack[url]:%s", receivePack["status"],receivePack["url"]);
                if(receivePack["status"] == 1){
                    that.sendAudioURLToServer(receivePack["url"]);
                }else{
                    console.log("sendRecordFile receivePack[status]=2 msg:%s", receivePack["msg"]);
                    app[app.subGameName + "_SysNotifyManager"]().ShowSysMsg('MSG_AUDIO_SEND_FAIL');
                }
            }
            else{
...
var g_nn_AudioManager = null;
  /**
  ...
  */
exports.GetModel = function(){
    if(!g_nn_AudioManager){
        g_nn_AudioManager = new nn_AudioManager();
    }
    return g_nn_AudioManager;
}