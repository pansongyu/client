...
var nn_EffectManager = app.BaseClass.extend({
    ...
    CreateEffectByEffectRes:function(effectName, effectRes, userData=0){
        ...
        return this.CreateEffect(effectName, effectRes, userData, prefabPath, componentName, zorderLv)
                    .catch(function(error){
                        console.error("CreateEffectByEffectRes(%s,%s,%s) error:%s", effectName, effectRes, userData, error.stack);
                    })
        ...
        if(effectNode){
            ...
            return this.LoadPrefab(prefabPath)
                        .then(function(effePrefabObj){
                            ...
                            let effectComponent = effectNode.getComponent(componentName);
                            if(!effectComponent){
                                console.error("CreateEffect(%s,%s) not find Component (%s)", effectName, effectRes, componentName);
                                return null
                            }
...
var g_nn_EffectManager = null;

/**
 * 绑定模块外部方法
 */
exports.GetModel = function(){
    if(!g_nn_EffectManager){
        g_nn_EffectManager = new nn_EffectManager();
    }
    return g_nn_EffectManager;
}