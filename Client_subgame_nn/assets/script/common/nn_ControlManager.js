...
var nn_ControlManager = app.BaseClass.extend({
    ...
    CreateLoadPromise:function(resPath, resType=""){
        ...
        let promisefunc = function(resolve, reject){
            ...
            cc.loader.loadRes(resPath, resType, function (error, loadData) {
                if(error){
                    reject(error);
                    console.error("CreateLoadPromise failed resPath(%s) resType(%s), error:%s", resPath, resType, error.stack);
                    console.error("CreateLoadPromise loadData(%s)", loadData);
...
var g_nn_ControlManager = null;

/**
 * 绑定模块外部方法
 */
exports.GetModel = function(){
    if(!g_nn_ControlManager){
        g_nn_ControlManager = new nn_ControlManager();
    }
    return g_nn_ControlManager;
}