...
var nn_SysDataManager = app.BaseClass.extend({
	...
	GetTableDict:function(tableName){
		...
		if (!this.tableDict[tableName]){
			console.error("GetTableDict not load  "+tableName);
			return {}
		}
	...
	GetJsonData:function(tableName){
		...
		if(!dataDict){
			console.error("GetJsonData tableName not find  "+tableName);
			return
		}
		return dataDict
	},
...
var g_nn_SysDataManager = null;

/**
 * 绑定模块外部方法
 */
exports.GetModel = function(){
    if(!g_nn_SysDataManager){
        g_nn_SysDataManager = new nn_SysDataManager();
    }
    return g_nn_SysDataManager;
}