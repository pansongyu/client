...
var nn_SceneManager = app.BaseClass.extend({
	...
	OnEvent_ModalLayer: function (event) {
		if (!this.sceneComponent) {
			console.warn("OnEvent_ModalLayer not sceneName");
			return
		}
	...
	OnEventHide: function () {
		if (!this.sceneComponent) {
			console.warn("OnEventHide not sceneName");
			return
		}
	...
	OnEventShow: function (bReConnect) {
		if (!this.sceneComponent) {
			console.warn("OnEventShow not sceneName");
			return
		}
	...
	ReadyLoadScene: function (sceneName, mapID) {
		...
		console.log("LoadScene((%s,%s) -> (%s,%s))", lastSceneName, this.mapID, sceneName, mapID);
		this.mapID = mapID;
		...
		if (bluebirdObj) {
			bluebirdObj.then(function () {
				...
				.catch(function (error) {
					console.error("LoadSceneDefaultForm(%s) error:%s", sceneName, error.stack);
	...
	RealGamePrefab: function (GamePrefab) {
		...
		if(is3DShow==0 && GamePrefab=='game/ZJMJ/ui/UIZJMJPlay'){
			 //XY场景需要切换
			 GamePrefab='game/ZJMJ/ui/UIZJMJXYPlay';
		}
	...
	OnLoadSceneEnd: function () {
		console.log("OnLoadSceneEnd sceneName:%s", this.sceneName);
...
var g_nn_SceneManager = null;

/**
 * 绑定模块外部方法
 */
exports.GetModel = function () {
	if (!g_nn_SceneManager) {
		g_nn_SceneManager = new nn_SceneManager();
	}
	return g_nn_SceneManager;
}
