...
var nn_WeChatManager = app.BaseClass.extend({
	...
	SetWeChatShareDataInfo:function(title, desc, argDict){
		if(!this.sdk){
			console.log("SetWeChatShareDataInfo not init sdk");
			return
		}
	...
	OnHttpPack_HeadImagePathInfo:function(serverPack){
		...
		app.bluebird.all(bluebirdList)
			...
			.catch(function(error){
				console.error("OnHttpPack_HeadImagePathInfo:%s", error.stack)
			})
		...
		return this.ControlManager.CreateLoadPromiseByUrl(targetPath)
			.then(function(texture){
				if(texture instanceof cc.Texture2D){
					...
					console.error("texture not Texture2D");
				}
			})
			.catch(function(error){
				console.error("OnHttpPack_HeadImagePathInfo error:%s", error.stack);
			})
	},
	...
	SendRequestWeChatSDKIntSign:function(){
		...
		//没有配置公众号,不需要分享功能
		if(!mpID){
			console.log("SendRequestWeChatSDKIntSign not mpID");
			return
		}
...
var g_nn_WeChatManager = null;

/**
 * 绑定模块外部方法
 */
exports.GetModel = function(){
	if(!g_nn_WeChatManager){
		g_nn_WeChatManager = new nn_WeChatManager();
	}
	return g_nn_WeChatManager;
}