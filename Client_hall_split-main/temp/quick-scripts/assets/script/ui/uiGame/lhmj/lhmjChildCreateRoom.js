(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/script/ui/uiGame/lhmj/lhmjChildCreateRoom.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '1a6efraG3ZFYYuU1nQpgj6X', 'lhmjChildCreateRoom', __filename);
// script/ui/uiGame/lhmj/lhmjChildCreateRoom.js
...
var lhmjChildCreateRoom = cc.Class({
	...
	CreateSendPack: function CreateSendPack(renshu, setCount, isSpiltRoomCard) {
		...
		var feng = [];
		for (var i = 0; i < this.Toggles['feng'].length; i++) {
			if (this.Toggles['feng'][i].getChildByName('checkmark').active) {
				feng.push(i);
			}
		}
		var guize = [];
		for (var _i = 0; _i < this.Toggles['guize'].length; _i++) {
			if (this.Toggles['guize'][_i].getChildByName('checkmark').active) {
				guize.push(_i);
			}
		}
		// let kexuanwanfa = [];
	...
	RefreshAllToggles: function RefreshAllToggles(gameType) {
		...
		for (var key in this.gameCreateConfig) {
			if (this.gameType == this.gameCreateConfig[key].GameName) {
				...
				var lastPos = { x: 0, y: 0 };
				for (var _i4 = 1; _i4 <= toggleCount; _i4++) {
					var curNode = node.getChildByName('Toggle' + _i4);
					...
					if (curNode) {
						...
						if (1 == _i4) {
							...
							if (AtRows[_i4 - 2] != AtRows[_i4 - 1]) {
								...
								// }
								curNode.x = lastPos.x + parseInt(spacing[_i4 - 1]);
								curNode.y = lastPos.y;
							}
						}
						lastPos.x = curNode.x;
	...
	UpdateOnClickToggle: function UpdateOnClickToggle() {
		if (this.Toggles["guize"]) {
			if (this.Toggles["xiapao"][0].getChildByName("checkmark").active) {
				this.Toggles["guize"][11].active = true;
			} else {
				this.Toggles["guize"][11].getChildByName("checkmark").active = false;
				this.Toggles["guize"][11].active = false;
			}
			//2人场不能勾选“闲家加底”
			if (this.Toggles["renshu"][0].getChildByName("checkmark").active) {
				this.Toggles["guize"][1].active = false;
				this.Toggles["guize"][1].getChildByName("checkmark").active = false;
			} else {
				this.Toggles["guize"][1].active = true;
			}
			//勾选报听后才能勾选报听有财
			if (this.Toggles["guize"][6].getChildByName("checkmark").active) {
				this.Toggles["guize"][7].active = true;
			} else {
				this.Toggles["guize"][7].active = false;
				this.Toggles["guize"][7].getChildByName("checkmark").active = false;
			}
			if (this.Toggles["guize"][4].getChildByName("checkmark").active) {
				this.Toggles["guize"][5].getChildByName("checkmark").active = false;
			}
			if (this.Toggles["guize"][5].getChildByName("checkmark").active) {
				this.Toggles["guize"][4].getChildByName("checkmark").active = false;
			}
			if (this.Toggles["moshi"][3].getChildByName("checkmark").active) {
				for (var i = 0; i < this.Toggles["guize"].length; i++) {
					if (i < 6) {
						this.Toggles["guize"][i].active = true;
					} else {
						this.Toggles["guize"][i].active = false;
						this.Toggles["guize"][i].getChildByName("checkmark").active = false;
					}
				}
				this.Toggles["feng"][0].parent.active = false;
				for (var _i5 = 0; _i5 < this.Toggles["feng"].length; _i5++) {
					this.Toggles["feng"][_i5].getChildByName("checkmark").active = false;
				}
			} else {
				for (var _i6 = 0; _i6 < this.Toggles["guize"].length; _i6++) {
					this.Toggles["guize"][_i6].active = true;
				}
				this.Toggles["feng"][0].parent.active = true;
			}
		}
		if (this.Toggles["feng"]) {
			if (this.Toggles["guize"][3].getChildByName("checkmark").active) {
				this.Toggles["feng"][1].active = false;
				this.Toggles["feng"][1].getChildByName("checkmark").active = false;
			} else {
				this.Toggles["feng"][1].active = true;
			}
			if (this.Toggles["feng"][2].getChildByName("checkmark").active) {
				this.Toggles["feng"][3].getChildByName("checkmark").active = false;
			}
			if (this.Toggles["feng"][3].getChildByName("checkmark").active) {
				this.Toggles["feng"][2].getChildByName("checkmark").active = false;
			}
			if (this.Toggles["feng"][0].getChildByName("checkmark").active) {
				this.Toggles["guize"][4].active = false;
				this.Toggles["guize"][4].getChildByName("checkmark").active = false;
				this.Toggles["guize"][5].active = false;
				this.Toggles["guize"][5].getChildByName("checkmark").active = false;
			} else {
				this.Toggles["guize"][4].active = true;
				this.Toggles["guize"][5].active = true;
			}
		}
	}
});

module.exports = lhmjChildCreateRoom;
...
cc._RF.pop();
        ...
        //# sourceMappingURL=lhmjChildCreateRoom.js.map
