(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/script/ui/uiGame/gzmj/gzmj_winlost_child.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, 'd616bWDKoRNLbutBuZkABst', 'gzmj_winlost_child', __filename);
// script/ui/uiGame/gzmj/gzmj_winlost_child.js
...
cc.Class({
	...
	ShowPlayerData: function ShowPlayerData(setEnd, playerAll, index) {
		...
		var huPaiDiFen = setEnd["huPaiDiFen"];
		...
		posResult["huPaiDiFen"] = huPaiDiFen;
  ...
  this.node.getChildByName("user_info").getChildByName("label_upLevel").getComponent(cc.Label).string = "所属推广员ID：" + upLevelId;*/
	...
	UpdatePlayData: function UpdatePlayData(PlayerNode, HuList, PlayerInfo) {
		...
		this.ShowPlayerJieSuan(PlayerNode.getChildByName('huTypeScrollView').getChildByName('view'), HuList);
  ...
  */
	...
	ShowPlayerJieSuan: function ShowPlayerJieSuan(ShowNode, huInfoAll) {
		var demo = this.node.getChildByName("demo_item");
		demo.active = false;
		var content = ShowNode.getChildByName("content");
		for (var i = 0; i < content.children.length; i++) {
			content.children[i].active = false;
		}
		var huInfo = huInfoAll['endPoint'];
		var huTypeMapB = huInfo["huTypeMapB"];
		var huPaiDiFen = huInfoAll["huPaiDiFen"];
		huTypeMapB["huPaiDiFen"] = huPaiDiFen;
		var huTypeDict = this.GetHuTypeDict();
		for (var huType in huTypeMapB) {
			var item = content.getChildByName(huType);
			if (!item) {
				item = cc.instantiate(demo);
				content.addChild(item);
				item.name = huType;
			}
			item.active = true;
			var huPoint = huTypeMapB[huType];
			this.ShowHuType(item, huTypeDict, huType, huPoint);
		}
  ...
  for (let huType in huInfo) {
	...
	ShowHuType: function ShowHuType(ShowNode, huTypeDict, huType, huPoint) {
  	...
  	// this.ShowLabelName(ShowNode.getChildByName("label_lists"), huTypeDict[huType] + "：" + huPoint + "X2");
  	...
  	this.ShowLabelName(ShowNode.getChildByName("label_lists"), huTypeDict[huType] + "：" + huPoint);
		...
		ShowNode.getComponent(cc.Label).string = huTypeDict[huType];
		if (huType == "huPaiDiFen") {
			huPoint = "X" + huPoint;
		} else {
			if (huPoint > 0) {
				huPoint = "+" + huPoint;
		...
		ShowNode.getChildByName("num").getComponent(cc.Label).string = huPoint;
	},
	//分数
...
cc._RF.pop();
        ...
        //# sourceMappingURL=gzmj_winlost_child.js.map
