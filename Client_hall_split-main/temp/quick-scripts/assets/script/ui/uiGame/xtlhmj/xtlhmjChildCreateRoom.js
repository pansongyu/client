(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/script/ui/uiGame/xtlhmj/xtlhmjChildCreateRoom.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, 'd76772f5rdFZZa/xzso04RP', 'xtlhmjChildCreateRoom', __filename);
// script/ui/uiGame/xtlhmj/xtlhmjChildCreateRoom.js
...
var xtlhmjChildCreateRoom = cc.Class({
	...
	CreateSendPack: function CreateSendPack(renshu, setCount, isSpiltRoomCard) {
		var sendPack = {};
		var laiziwanfa = this.GetIdxByKey('laiziwanfa');
		var kexuanwanfa = this.GetIdxsByKey('kexuanwanfa');
		var suopai = this.GetIdxByKey('suopai');
	...
	/*AdjustSendPack: function (sendPack) {
 	// u 勾选“无鬼”玩法，不能勾选“无鬼加倍”；
 	if (sendPack.guipai == 0) {
 		this.RemoveMultiSelect(sendPack, "kexuanwanfa", 15);
 	}
 	// 不分庄闲：仅在勾选“只能自摸”情况下可选
 	if (sendPack.mapai == 0 || sendPack.mapai == 2) {
 		this.RemoveRadioSelect(sendPack, "mashu");
 	}
 	if (sendPack.mapai == 0) {
 		this.RemoveRadioSelect(sendPack, "guize");
 	}
 	return sendPack;
 },
 ...
 UpdateOnClickToggle: function () {
 	...
 	if (this.Toggles["mashu"]) {
 		if (this.Toggles["mapai"][0].getChildByName("checkmark").active || this.Toggles["mapai"][2].getChildByName("checkmark").active) {
 	...
 	if (this.Toggles["guize"]) {
 		if (this.Toggles["mapai"][2].getChildByName("checkmark").active) {
 			this.Toggles["guize"][2].active = false;
 			if (this.Toggles["guize"][2].getChildByName("checkmark").active) {
 				this.Toggles["guize"][2].getChildByName("checkmark").active = false;
 				this.Toggles["guize"][0].getChildByName("checkmark").active = true;
 			}
 		} else {
 			this.Toggles["guize"][2].active = true;
 		}
 		if (this.Toggles["mapai"][1].getChildByName("checkmark").active) {
 			this.Toggles["guize"][1].active = false;
 			if (this.Toggles["guize"][1].getChildByName("checkmark").active) {
 		...
 		if (this.Toggles["mapai"][0].getChildByName("checkmark").active) {
 			this.Toggles["guize"][0].parent.active = false;
 		} else {
 			this.Toggles["guize"][0].parent.active = true;
 		}
 	}
 },*/
});

module.exports = xtlhmjChildCreateRoom;
...
cc._RF.pop();
        ...
        //# sourceMappingURL=xtlhmjChildCreateRoom.js.map
