(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/script/ui/uiGame/klmj/UIRecordAllResult_klmj.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, 'd300flR64pIXoljQlWaWe5R', 'UIRecordAllResult_klmj', __filename);
// script/ui/uiGame/klmj/UIRecordAllResult_klmj.js
...
cc.Class({
	...
	InitShowPlayerInfo: function InitShowPlayerInfo(setEnd) {
		...
		var jiangList = setEnd["jiangList"] || [];
		var zhongJiangList = setEnd["zhong1List"] || [];

		var jin1 = setEnd["jin"] || 0;
		var jin2 = setEnd["jin2"] || 0;

		this.ShowMaiMaPai(maList, zhongList, jin1, jin2);
		this.ShowJiangMaPai(jiangList, zhongJiangList, jin1, jin2);

		this.ShowJinNode(jin1, jin2);
	},
	ShowMaiMaPai: function ShowMaiMaPai(cardIDList, zhongList, jin1, jin2) {
		var bg_maima = this.node.getChildByName("mapai").getChildByName("bg_maima");
		if (cardIDList.length == 0) {
			bg_maima.active = false;
			return;
		}
		bg_maima.active = true;
		var niaopaiNode = bg_maima.getChildByName("niaopai");
		for (var i = 0; i < niaopaiNode.children.length; i++) {
			niaopaiNode.children[i].active = false;
		}
		for (var _i3 = 0; _i3 < cardIDList.length; _i3++) {
			var node = niaopaiNode.getChildByName("card" + (_i3 + 1));
			this.ShowImage(node, cardIDList[_i3]);
			node.active = true;
			node.getChildByName("da").active = Math.floor(cardIDList[_i3] / 100) == Math.floor(jin1 / 100) || Math.floor(cardIDList[_i3] / 100) == Math.floor(jin2 / 100);
			node.color = zhongList.indexOf(cardIDList[_i3]) == -1 ? cc.color(180, 180, 180) : cc.color(255, 255, 255);
		}
	},
	ShowJiangMaPai: function ShowJiangMaPai(cardIDList, zhongList, jin1, jin2) {
		var bg_jiangma = this.node.getChildByName("mapai").getChildByName("bg_jiangma");
		if (cardIDList.length == 0) {
			bg_jiangma.active = false;
			return;
		}
		bg_jiangma.active = true;
		var niaopaiNode = bg_jiangma.getChildByName("niaopai");
		for (var i = 0; i < niaopaiNode.children.length; i++) {
			niaopaiNode.children[i].active = false;
		}
		for (var _i4 = 0; _i4 < cardIDList.length; _i4++) {
			var node = niaopaiNode.getChildByName("card" + (_i4 + 1));
			this.ShowImage(node, cardIDList[_i4]);
			node.active = true;
			node.getChildByName("da").active = Math.floor(cardIDList[_i4] / 100) == Math.floor(jin1 / 100) || Math.floor(cardIDList[_i4] / 100) == Math.floor(jin2 / 100);
			node.color = zhongList.indexOf(cardIDList[_i4]) == -1 ? cc.color(180, 180, 180) : cc.color(255, 255, 255);
		}
	},
	ShowJinNode: function ShowJinNode(jin1, jin2) {
		var jinNode = this.node.getChildByName("jinNode");
		jinNode.getChildByName("jin1").active = jin1 != 0;
		jinNode.getChildByName("jin2").active = jin2 != 0;

		this.ShowImage(jinNode.getChildByName("jin1"), jin1);
		this.ShowImage(jinNode.getChildByName("jin2"), jin2);
	},
...
cc._RF.pop();
        ...
        //# sourceMappingURL=UIRecordAllResult_klmj.js.map
