(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/script/ui/uiGame/sjsss/sjsssChildCreateRoom.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '7d2bdGkuDlLdp0ecd//6bPB', 'sjsssChildCreateRoom', __filename);
// script/ui/uiGame/sjsss/sjsssChildCreateRoom.js
...
var sjsssChildCreateRoom = cc.Class({
	...
	EditBoxChanged: function EditBoxChanged() {
		var str = this.Toggles["wangpai"][0].getChildByName("editbox").getComponent(cc.EditBox).string;
		if (str && parseInt(str) > 20) {
			this.Toggles["wangpai"][0].getChildByName("editbox").getComponent(cc.EditBox).string = "20";
		}
		if (str && parseInt(str) < 0) {
			this.Toggles["wangpai"][0].getChildByName("editbox").getComponent(cc.EditBox).string = "0";
		}
	},
	...
	Click_btn_create: function Click_btn_create(createType) {
		...
		if (3 == this.sign) {
			var wangpai = this.Toggles["wangpai"][0].getChildByName("editbox").getComponent(cc.EditBox).string;
			var reg = /^[1-9]\d*$/;
			if (!reg.test(wangpai)) {
				app.SysNotifyManager().ShowSysMsg("请输入0-20之间的王牌数量");
				return;
			}
			if (parseInt(wangpai) < 0 || parseInt(wangpai) > 20) {
				app.SysNotifyManager().ShowSysMsg("请输入0-20之间的正整数");
				return;
			}
			var mapai = this.GetIdxByKey("mapai");
			if (mapai == 1 && !this.maPai) {
				app.SysNotifyManager().ShowSysMsg("请选择马牌");
				return;
			}
		}
	...
	CreateSendPack: function CreateSendPack(renshu, setCount, isSpiltRoomCard) {
		...
		var huase = [];
		for (var _i6 = 0; _i6 < this.Toggles["huase"].length; _i6++) {
			if (this.Toggles["huase"][_i6].getChildByName("checkmark").active) {
				huase.push(_i6);
			}
		}
		var speed = this.GetIdxByKey("speed");

		var mapai = this.GetIdxByKey("mapai");

		//打枪倍数
		var daqiang = 0; //this.GetIdxByKey("daqiang");
		//模式
		var moshi = 0; //this.GetIdxByKey("moshi");
		//王牌
		var wangpai = this.GetIdxByKey("wangpai");
		//底分
		var difen = 0; //this.GetIdxByKey("difen");
		//限时
		var xianShi = this.GetIdxByKey("xianShi");
		//解散
		var jiesan = this.GetIdxByKey("jiesan");
		if (mapai != 0 && this.maPai) {
			mapai = this.maPai;
		}
	...
	ShowTogglesBySign: function ShowTogglesBySign() {
		...
		this.Toggles["wangpai"][0].getChildByName("editbox").x = 90;
		this.Toggles["wangpai"][0].getChildByName("editbox").getComponent(cc.EditBox).string = "20";
	},
	OnToggleClick: function OnToggleClick(event) {
		...
		if ("jushu" == key || "renshu" == key || "fangfei" == key) {
			...
			if (toggleIndex == 1) {
				app.FormManager().ShowForm("UISelectMaPai", function (mp) {
					_this.maPai = mp;
					_this.UpdateTxt();
				}, this.maPai);
			}
	...
	UpdateTxt: function UpdateTxt() {
		var lb = this.Toggles["mapai"][1].getChildByName("label").getComponent(cc.Label);
		if (!this.maPai) {
			lb.string = "未选择";
			return;
		}
		...
		var name = "当前选择：" + (colors[Math.floor(this.maPai / 16)] || "") + (values[this.maPai % 16] || this.maPai % 16);
		lb.string = name;
	},
	UpdateSj: function UpdateSj() {
		var kexuanwanfa = [];
		if (!this.Toggles["kexuanwanfa"] || !this.Toggles["kexuanwanfa"].length) {
			return;
		}
		for (var i = 0; i < this.Toggles["kexuanwanfa"].length; i++) {
			if (this.Toggles["kexuanwanfa"][i].getChildByName("checkmark").active) {
		...
		if (kexuanwanfa.indexOf(1) >= 0) {
			for (var index = 5; index < 11; index++) {
				var element = this.Toggles["renshu"][index];
				element.active = false;
			}
			var renshu = this.getCurSelectRenShu();
			cc.log(renshu);
			if (renshu[0] > 6) {
				for (var _i7 = 0; _i7 < this.Toggles['renshu'].length; _i7++) {
					var toggle = this.Toggles['renshu'][_i7].getChildByName('checkmark');
					if (_i7 == 0) toggle.active = true;else toggle.active = false;
				}
			}
		} else {
			for (var _index = 5; _index < 11; _index++) {
				var _element = this.Toggles["renshu"][_index];
...
module.exports = sjsssChildCreateRoom;
...
cc._RF.pop();
        ...
        //# sourceMappingURL=sjsssChildCreateRoom.js.map
