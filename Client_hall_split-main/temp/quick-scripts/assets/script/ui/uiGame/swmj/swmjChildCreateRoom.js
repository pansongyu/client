(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/script/ui/uiGame/swmj/swmjChildCreateRoom.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '076bdUITPFN25lSei0YdZl5', 'swmjChildCreateRoom', __filename);
// script/ui/uiGame/swmj/swmjChildCreateRoom.js
...
var fzmjChildCreateRoom = cc.Class({
	...
	CreateSendPack: function CreateSendPack(renshu, setCount, isSpiltRoomCard) {
		var sendPack = {};
		var fenghua = this.GetIdxByKey('fenghua');
		var kexuanwanfa = this.GetIdxByKeyMulti('kexuanwanfa');
		var guipai = this.GetIdxByKey('guipai');
		var teshupaixing = [];
		if (guipai > 0) {
			teshupaixing = this.GetIdxByKeyMulti('teshupaixing');
		}
		var jiangma = this.GetIdxByKey('jiangma');
		var mafen = this.GetIdxByKey('mafen');
		var paohu = this.GetIdxByKeyMulti('paohu');
		var paixingfanbei = this.GetIdxByKeyMulti('paixingfanbei');
	...
	GetIdxByKeyMulti: function GetIdxByKeyMulti(key) {
		var xuanzhong = [];
	...
	OnToggleClick: function OnToggleClick(event) {
		...
		if ('jushu' == key || 'renshu' == key || 'fangfei' == key) {
			...
			return;
		} else if ('paixingfanbei' == key) {
			if (toggleIndex == 0) {
				if (this.Toggles['paixingfanbei'][0].getChildByName('checkmark').active == false) {
					this.Toggles['teshupaixing'][0].getChildByName('checkmark').active = false;
					this.UpdateLabelColor(this.Toggles['teshupaixing'][0].parent);
				}
			}
			if (toggleIndex == 1) {
				if (this.Toggles['paixingfanbei'][1].getChildByName('checkmark').active == false) {
					this.Toggles['teshupaixing'][1].getChildByName('checkmark').active = false;
					this.UpdateLabelColor(this.Toggles['teshupaixing'][0].parent);
				}
			}
			/*if(toggleIndex == 8){
   	if(this.Toggles['paixingfanbei'][8].getChildByName('checkmark').active==false){
   		this.Toggles['paixingfanbei'][9].getChildByName('checkmark').active=false;
   	}
   }
   if(toggleIndex == 9){
   	if(this.Toggles['paixingfanbei'][9].getChildByName('checkmark').active==false){
   		this.Toggles['paixingfanbei'][8].getChildByName('checkmark').active=false;
   	}
   }*/
		} else if ('teshupaixing' == key) {
			if (toggleIndex == 0) {
				if (this.Toggles['teshupaixing'][0].getChildByName('checkmark').active == false) {
					this.Toggles['paixingfanbei'][0].getChildByName('checkmark').active = false;
					this.UpdateLabelColor(this.Toggles['paixingfanbei'][0].parent);
				}
			}
			if (toggleIndex == 1) {
				if (this.Toggles['teshupaixing'][1].getChildByName('checkmark').active == false) {
					this.Toggles['paixingfanbei'][1].getChildByName('checkmark').active = false;
					this.UpdateLabelColor(this.Toggles['paixingfanbei'][0].parent);
				}
			}
		}
	...
	ReShowToggle: function ReShowToggle() {
		if (this.Toggles['guipai']) {
			//无鬼隐藏特殊牌型
			if (this.Toggles['teshupaixing']) {
				this.Toggles['teshupaixing'][0].parent.active = !this.Toggles['guipai'][0].getChildByName('checkmark').active;
				this.UpdateLabelColor(this.Toggles['teshupaixing'][0].parent);
			}
		...
		if (this.Toggles['kexuanwanfa']) {
			if (this.Toggles['kexuanwanfa'][7].getChildByName('checkmark').active == true) {
...
cc._RF.pop();
        ...
        //# sourceMappingURL=swmjChildCreateRoom.js.map
