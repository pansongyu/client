...
cc.Class({
    ...
    OnShow: function () {
        ...
        // if(BackVolume){
            //如果背景音量大于0.读取缓存背景音乐
            let MainBackMusic=this.LocalDataManager.GetConfigProperty("SysSetting","MainBackMusic");
    ...
    ShowHero_NameOrID:function () {
        ...
        if(this.lb_name.node.width > 210 && heroName.length > 7)
            this.lb_name.string = this.lb_name.string.substring(0,7) + '...';
        let heroID = app.HeroManager().GetHeroProperty("pid");
        let WeChatHeadImage1 = this.node_head.getComponent("WeChatHeadImage");
    ...
    Click_Change_Back_Music:function(backGroundSound){
        let SceneManager = app.SceneManager();
    ...
    Click_Btn_YinSi:function () {
        this.FormManager.ShowForm("UIYinSiZhenCe");
    },

    Click_Btn_FuWu:function () {
