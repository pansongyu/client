...
var twmjChildCreateRoom = cc.Class({
	...
	CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
		...
		let di = this.GetIdxByKey("di");
		let tai = this.GetIdxByKey("tai");
		let wanfa = this.GetIdxByKey("wanfa");
		let kaimenxu = this.GetIdxByKey("kaimenxu");
	...
	UpdateTogglesLabel: function (TogglesNode, isResetPos = true) {
		if (this.Toggles["tai"]) {
			for (let j = 0; j < this.Toggles["tai"].length; j++) {
			...
			for (let i = 0; i < this.Toggles["tai"].length; i++) {
					...
					if ('renshu' == curKey || 'fangfei' == curKey || 'jushu' == curKey) {
						...
						if (this.Toggles['jushu']) {
							...
							for (let i = 0; i < curCostData.length; i++) {
								if (curCostData[i].SetCount == 401) {
									this.Toggles['jushu'][i].getChildByName('label').getComponent(cc.Label).string = "1圈";
								} else if (curCostData[i].SetCount == 404) {
									this.Toggles['jushu'][i].getChildByName('label').getComponent(cc.Label).string = "1将";
								} else {
