...
var wzmjChildCreateRoom = cc.Class({
                ...
                if(curKey == this.gameCreateConfig[key].Key){
                    ...
                    if ('yushu' == curKey) {
                        ...
                        if (this.Toggles["diaoyu"][2].getChildByName("checkmark").active ||
                            this.Toggles["diaoyu"][3].getChildByName("checkmark").active) {
                            this.Toggles["fengyupai"][0].parent.active = true;
                        } else {
                            this.Toggles["fengyupai"][0].parent.active = false;
                        }
    ...
    CreateSendPack:function(renshu, setCount, isSpiltRoomCard){
        ...
        let diaoyu = this.GetIdxByKey('diaoyu');
        let kexuanwanfa = [];
        ...
        let yushu = this.GetIdxByKey('yushu');
        if (diaoyu == 2 || diaoyu == 3) {
            yushu = -1;
        }
        let fengyupai = this.GetIdxByKey('fengyupai');
        if (diaoyu == 0) {
            fengyupai = -1;
        }
        sendPack = {
