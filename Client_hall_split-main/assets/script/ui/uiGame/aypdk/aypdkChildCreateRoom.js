...
var aypdkChildCreateRoom = cc.Class({
    ...
    CreateSendPack:function(renshu, setCount, isSpiltRoomCard){
        let sendPack = {};
        let chupai = this.GetIdxByKey('chupai');
        let heitaosanbichu = this.GetIdxByKey('heitaosanbichu');
        let zhadan = this.GetIdxByKey('zhadan');
        let xianShi = this.GetIdxByKey('xianShi');
        let daxiaoguan = this.GetIdxByKey('daxiaoguan');
        ...
        for(let i=0;i<this.Toggles['paixing'].length;i++){
            let isCheck = this.Toggles['paixing'][i].getChildByName('checkmark').active;
            if(isCheck){
                paixing.push(i);
            }
        }
        for(let i=0;i<this.Toggles['teshu'].length;i++){
            let isCheck = this.Toggles['teshu'][i].getChildByName('checkmark').active;
            if(isCheck){
                teshu.push(i);
            }
        }
