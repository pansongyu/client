...
var xgkwxmjChildCreateRoom = cc.Class({
	...
	CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
		...
		let jiapiao = this.GetIdxByKey("jiapiao");
		let chuzi = this.GetIdxByKey("chuzi");
		let baohu = this.GetIdxByKey("baohu");
		...
		let maima = [];
		for (let i = 0; i < this.Toggles['maima'].length; i++) {
			if (this.Toggles['maima'][i].getChildByName('checkmark').active) {
				maima.push(i);
			}
		}
		let shukan = [];
		for (let i = 0; i < this.Toggles['shukan'].length; i++) {
			if (this.Toggles['shukan'][i].getChildByName('checkmark').active) {
				shukan.push(i);
			}
		}
		let fangjian = [];
