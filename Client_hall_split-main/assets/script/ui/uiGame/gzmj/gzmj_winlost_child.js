...
cc.Class({
	...
	ShowPlayerData: function (setEnd, playerAll, index) {
		...
		let huPaiDiFen = setEnd["huPaiDiFen"];
	...
	ShowPlayerJieSuan: function (ShowNode, huInfoAll) {
		let demo = this.node.getChildByName("demo_item");
		demo.active = false;
		let content = ShowNode.getChildByName("content");
		for (let i = 0; i < content.children.length; i++) {
			content.children[i].active = false;
		}
		let huInfo = huInfoAll['endPoint'];
		let huTypeMapB = huInfo["huTypeMapB"];
		let huPaiDiFen = huInfoAll["huPaiDiFen"];
		huTypeMapB["huPaiDiFen"] = huPaiDiFen;
		let huTypeDict = this.GetHuTypeDict();
		for (let huType in huTypeMapB) {
			let item = content.getChildByName(huType);
			if (!item) {
