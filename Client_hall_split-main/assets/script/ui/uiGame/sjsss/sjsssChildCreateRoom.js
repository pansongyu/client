...
var sjsssChildCreateRoom = cc.Class({
		...
		CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
			...
			let huase = [];
			for (let i = 0; i < this.Toggles["huase"].length; i++) {
				if (this.Toggles["huase"][i].getChildByName("checkmark").active) {
					huase.push(i);
				}
			}
			let speed = this.GetIdxByKey("speed");

			let mapai = this.GetIdxByKey("mapai");

			//打枪倍数
			let daqiang = 0//this.GetIdxByKey("daqiang");
			//模式
			let moshi = 0//this.GetIdxByKey("moshi");
			//王牌
			let wangpai = this.GetIdxByKey("wangpai");
			//底分
			let difen = 0//this.GetIdxByKey("difen");
			//限时
		...
		OnToggleClick: function (event) {
			...
			if ("jushu" == key || "renshu" == key || "fangfei" == key) {
				...
				if (toggleIndex == 1) {
					app.FormManager().ShowForm("UISelectMaPai", (mp)=>{
						this.maPai = mp
						this.UpdateTxt()
					},this.maPai)	
				}
		...
		UpdateTxt : function () {
			let lb = this.Toggles["mapai"][1].getChildByName("label").getComponent(cc.Label)
			if (!this.maPai) {
				lb.string = "未选择"
				return
			}
			...
			let name = "当前选择：" + (colors[Math.floor(this.maPai/16)] || "") + (values[this.maPai%16] || this.maPai%16)
			lb.string = name
		},
		UpdateSj : function(){
			let kexuanwanfa = [];
			...
			if (kexuanwanfa.indexOf(1) >= 0) {
				for (let index = 5; index < 11; index++) {
					const element = this.Toggles["renshu"][index];
				...
				if (renshu[0] > 6) {
					for (let i = 0; i < this.Toggles['renshu'].length; i++) {
						...
						if(i == 0) toggle.active = true
						else toggle.active = false
					}
				}
			}
			else{
