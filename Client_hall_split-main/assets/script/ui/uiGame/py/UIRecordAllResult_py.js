...
cc.Class({
    ...
    OnShow: function (roomId, playerAll, gameType, unionId = 0, roomKey = null, defaultPage = 1) {
        ...
        //pk牌的type需要使用麻将的布局
        let gameNameList = ["bw", "pypp", "ctwsk", "ygwsk", "qcdg", "tsdg", "dhd", "ysdz", "wsk", "sdh", "lkwsk", "nzdl", "hndzp", "gxcdd", "glpp", "nzwsk", "gz", "xsy", "ctpk", "fczha", "xywsk","jxyz","qwwes", "bzp", "tgwsk"];
        //特殊处理 包王垂直布局
    ...
    InitShowPlayerInfo: function (setEnd) {
        ...
        for (let i = 0; i < posResultList.length; i++) {
            if (!posResultList[i].pid || pidList.indexOf(posResultList[i].pid) > -1) continue;
            pidList.push(posResultList[i].pid);
