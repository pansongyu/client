...
var fzmjChildCreateRoom = cc.Class({
    ...
    CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
        ...
        let zhuaniaomoshi = this.GetIdxByKey('zhuaniaomoshi');
        let niaoshu = this.GetIdxByKey('niaoshu');
        let zhongniao = this.GetIdxByKey('zhongniao');
        let fanbei = this.GetIdxByKey('fanbei');
        ...
        if (zhuaniaomoshi == 3) { // 不抓鸟
            niaoshu = -1;
            zhongniao = -1;
        }

        // 一鸟全中,不抓鸟
        if (zhuaniaomoshi == 2) {// 一鸟全中
            niaoshu = -1;
        }
    ...
    UpdateTogglesLabel: function (TogglesNode, isResetPos = true) {
        ...
        if (this.Toggles["zhongniao"]) {
            if (this.Toggles["zhuaniaomoshi"][3].getChildByName("checkmark").active) { // 不抓鸟
        ...
        // 勾选黑胡13，必选红中赖子11
        if (this.Toggles["kexuanwanfa"]) {
            if (this.Toggles["kexuanwanfa"][13].getChildByName("checkmark").active) { // 黑胡
    ...
    RefreshAllToggles: function (gameType) {
        ...
        if (zhuaNiaoToggleIndex == 2) {//选择一码全中，下方能鸟数不可见或不可选
            this.Toggles['niaoshu'][0].parent.active = false;
        } else {
            this.Toggles['niaoshu'][0].parent.active = true;
        }
    ...
    OnToggleClick: function (event) {
        ...
        if ('jushu' == key || 'renshu' == key || 'fangfei' == key || "zhuaniaomoshi" == key) {
            ...
            let zhuaniaomoshi1 = this.Toggles['zhuaniaomoshi'][1].getChildByName('checkmark').active;  // 全方位鸟
            if (zhuaniaomoshi1) { // 选择全方位鸟 抓鸟模式后不能切换人数
                let changePlayerNumIdx = 0;
                let toggle = this.Toggles['fangjian'][changePlayerNumIdx];
            ...
            if ('renshu' == key && !renshu4) {
                let fenBingIdx = 1;		// 全方位鸟
                let toggle = this.Toggles['zhuaniaomoshi'][fenBingIdx];
                let isCurSelect = toggle.getChildByName('checkmark').active;
                toggle.getChildByName('checkmark').active = false;
                this.UpdateLabelColor(toggle.parent);
                this.UpdateTogglesLabel(toggle.parent);

                if (isCurSelect) {// 全方位鸟为选中状态时
                    // 默认159中鸟
                    let toggle2 = this.Toggles['zhuaniaomoshi'][0];
        ...
        if ('kexuanwanfa' == key) {
            ...
            let qiXiaoDuIdx = 1;		// 七小对
            let qiDuIdx_1 = 2;			// 豪华七对
            let qiDuIdx_2 = 3;			// 双豪华七对
            let qiDuIdx_3 = 4;			// 三豪华七对

            let showIdxs1 = [];
            let tipContent = "";
            if (toggleIndex == 1 && !this.Toggles['kexuanwanfa'][1].getChildByName('checkmark').active) {		// 七小对
                showIdxs1 = [qiDuIdx_1, qiDuIdx_2, qiDuIdx_3];
                tipContent = "豪华七对、双豪华七对、三豪华七对任一玩法勾选后，默认有七小对";
            } else if (toggleIndex == 2 && !this.Toggles['kexuanwanfa'][2].getChildByName('checkmark').active) {		// 豪华七对
                showIdxs1 = [qiDuIdx_2, qiDuIdx_3];
                tipContent = "双豪华七对、三豪华七对任一玩法勾选后，默认有豪华七对";
            } else if (toggleIndex == 3 && !this.Toggles['kexuanwanfa'][3].getChildByName('checkmark').active) {		// 双豪华七对
                showIdxs1 = [qiDuIdx_3];
                tipContent = "三豪华七对勾选后，默认有双豪华七对"
            }

            for (let index = 0; index < showIdxs1.length; index++) {
                let idx = showIdxs1[index];
                let isSelect = this.Toggles['kexuanwanfa'][idx].getChildByName('checkmark').active;
                if (isSelect) {
            ...
            // 默认
            let showIdxs = [];
            if (this.Toggles['kexuanwanfa'][2].getChildByName('checkmark').active) {		// 豪华七对
                showIdxs = [qiXiaoDuIdx];
            }
            if (this.Toggles['kexuanwanfa'][3].getChildByName('checkmark').active) {		// 双豪华七对
                showIdxs = [qiXiaoDuIdx, qiDuIdx_1];
            }
            if (this.Toggles['kexuanwanfa'][4].getChildByName('checkmark').active) {		// 三豪华七对
                showIdxs = [qiXiaoDuIdx, qiDuIdx_1, qiDuIdx_2];
            }

            for (let i = 0; i < showIdxs.length; i++) {
                let idx = showIdxs[i];
                let toggle = this.Toggles['kexuanwanfa'][idx];
            ...
            let toggleIndexs = [6, 18, 19];
            if (toggleIndexs.indexOf(toggleIndex) > -1) {
                for (let idx = 0; idx < toggleIndexs.length; idx++) {
                    let tmpToggleIndex = toggleIndexs[idx];
                    let toggle = this.Toggles['kexuanwanfa'][tmpToggleIndex];
                    toggle.getChildByName('checkmark').active = tmpToggleIndex == toggleIndex;
                    this.UpdateLabelColor(toggle.parent);
                    this.UpdateTogglesLabel(toggle.parent);
                }
                // return;
