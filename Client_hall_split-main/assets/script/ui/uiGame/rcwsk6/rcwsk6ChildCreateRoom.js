...
var rcwskChildCreateRoom = cc.Class({
	...
	CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
        ...
        let dangshu = this.GetIdxByKey("dangshu");
        let weizhi = this.GetIdxByKey("weizhi");
        let paishu = [];
        for (let i = 0; i < this.Toggles['paishu'].length; i++) {
            if (this.Toggles['paishu'][i].getChildByName('checkmark').active) {
                paishu.push(i);
            }
        }
        let jiefeng = this.GetIdxByKey("jiefeng");
        let gou = this.GetIdxByKey("gou");
        let yufen = this.GetIdxByKey("yufen");
        let dangdifen = this.GetIdxByKey("dangdifen");
        let goudifen = this.GetIdxByKey("goudifen");
                        ...
                        if(this.Toggles['jushu']){
                            ...
                            for(let i=0; i<curCostData.length; i++){
                                if (curCostData[i].SetCount > 100) {
                                    this.Toggles['jushu'][i].getChildByName('label').getComponent(cc.Label).string = curCostData[i].SetCount % 100 + '档';
                                    continue;
                                }
