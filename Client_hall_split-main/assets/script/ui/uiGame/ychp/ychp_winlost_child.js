...
cc.Class({
    ...
    IsHuaJing: function (cardId) {
        let cardIds = this.GetHuaJingCardTypeCfg();
        // let cardType = Math.floor(cardId / 100);
        return cardIds.indexOf(cardId) > -1;
    },

    GetHuaJingCardTypeCfg: function () {
        if (this.wanfa == 0) {
            // 乙、三、五、七、九
    ...
    ShowOutCardImage: function (childNode) {
        childNode.active = true;
        let imagePath = "ui/uiGame/ychp/cards/out/" + Math.floor(childNode.cardID / 100);
        let that = this;
