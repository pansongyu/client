...
var wzmjChildCreateRoom = cc.Class({
    ...
    CreateSendPack:function(renshu, setCount, isSpiltRoomCard){
        ...
        let yipaoduoxiangzhuaniao = this.GetIdxByKey('yipaoduoxiangzhuaniao');
        let hzmj_wanfa = this.GetIdxByKey('wanfa');
        let xianShi = this.GetIdxByKey('xianShi');
        let jiesan = this.GetIdxByKey('jiesan');
        let tuoguan = this.GetIdxByKey('tuoguan');
        let suanFen = this.GetIdxByKey('suanFen');
        let suanFenNum = parseFloat(this.Toggles['suanFen'][1].getChildByName('editbox').getComponent(cc.EditBox).string);
        let hzmj_kexuanwanfa=[];
        ...
        let zhuaMas = this.GetIdxByKey('zhuaMas');
        sendPack = {
            "moban": moban,
