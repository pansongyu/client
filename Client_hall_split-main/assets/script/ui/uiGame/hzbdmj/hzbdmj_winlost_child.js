...
cc.Class({
    ...
    ShowPlayerShowCard: function (ShowNode, cardIDList, handCard) {
        ShowNode.active = 1;
    ...
    ShowPlayerNiaoPai:function(ShowNode,zhuaNiaoList,endPoint){
        let niaoPaiList = endPoint["zhongList"] || [];
        for(let i=1;i<=20;i++){
        ...
        for(let i=0;i<zhuaNiaoList.length;i++){
            let cardType= Math.floor(zhuaNiaoList[i] / 100);
            let node=ShowNode.getChildByName("card"+(i+1));
            this.ShowImage(node, 'EatCard_Self_', cardType);
            node.active=true;
            //更改为没中码都显示码牌
            if(niaoPaiList.length>0){
                if(niaoPaiList.indexOf(zhuaNiaoList[i]) > -1){
