...
var thgjmjChildCreateRoom = cc.Class({
    ...
    UpdateTogglesLabel:function (TogglesNode, isResetPos=true) {
        if(this.Toggles["renshu"]){
            let isHide = this.Toggles["renshu"][0].getChildByName("checkmark").active;
            let hideIdxs = [3, 4, 5];   //     财神、边卡吊、带风仅互报互飞可选；
            for (let i = 0; i < hideIdxs.length; i++) {
                let idx = hideIdxs[i];
                let toggle = this.Toggles['jushu'][idx];
                toggle.active = !isHide;
            }
            for (let i = 0; i < this.Toggles["jushu"].length; i++) {
                if(this.Toggles["jushu"][i].getChildByName("checkmark").active){
                    if(i>2){
