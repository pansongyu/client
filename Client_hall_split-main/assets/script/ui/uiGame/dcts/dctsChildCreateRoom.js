...
var ctwskChildCreateRoom = cc.Class({
                ...
                if (curKey == this.gameCreateConfig[key].Key) {
                    ...
                    if ('renshu' == curKey || 'fangfei' == curKey || 'jushu' == curKey || 'moshi' == curKey) {
                        ...
                        if (this.Toggles['jushu']) {
                            ...
                            // }
                            let moShiCosts = [];
                            if (this.moShiToggleIndex == 0) {//普通模式
                                moShiCosts = this.puTongCosts;
                            }else{
                                moShiCosts = this.biShangCosts;
                            }
                            for (let i = 0; i < moShiCosts.length; i++) {
                                this.Toggles['jushu'][i].getChildByName('label').getComponent(cc.Label).string = moShiCosts[i].SetCount + '局';
                            }
                        }
                        if (this.Toggles['fangfei'] && -1 != jushuIndex) {
                            ...
                            if (this.moShiToggleIndex == 1) {//比赏模式
    ...
    Click_btn_create: function (createType, isCheckedMic = false) {
        ...
        let setCount = this.puTongCosts[jushuIndex].SetCount;
