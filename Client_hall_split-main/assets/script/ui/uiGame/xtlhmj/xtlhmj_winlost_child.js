...
cc.Class({
	...
	ShowPlayerData: function (setEnd, playerAll, index) {
		...
		let maList = setEnd["zhuaNiaoList"] || [];
		let zhongMaList = setEnd["zhongNiaoList"] || [];
	...
	ShowPlayerAllScore: function (ShowNode, huInfo) {
		let piaoFen = huInfo["piaoFen"];
		let xiaoFen = huInfo["xiaoFen"];
		let huFen = huInfo["huFen"];
		if (piaoFen >= 0) {
			piaoFen = "+" + piaoFen;
		}
		if (xiaoFen >= 0) {
		...
		let str = "飘分:" + piaoFen + "  " + "笑分:" + xiaoFen + "  " + "胡分:" + huFen;
