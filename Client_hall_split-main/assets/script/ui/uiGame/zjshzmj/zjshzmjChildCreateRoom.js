...
var gamjChildCreateRoom = cc.Class({
    ...
    OnToggleClick:function(event){
        ...
        if('jushu' == key || 'renshu' == key || 'fangfei' == key){
			...
			if (toggleIndex == 0) {// 点击 七对子
				let is6 = this.Toggles['kexuanwanfa'][6].getChildByName('checkmark').active;
				let is0 = this.Toggles['kexuanwanfa'][0].getChildByName('checkmark').active;
				if (is0 && is6) {
			...
			if (toggleIndex == 1) {// 点击 爆头
				let is7 = this.Toggles['kexuanwanfa'][6].getChildByName('checkmark').active;
				let is6 = this.Toggles['kexuanwanfa'][1].getChildByName('checkmark').active;
				if (is7 && is6) {
					this.ShowSysMsg("勾选七对翻倍, 必须勾选七小对");
					return;
				}
			}
			if (toggleIndex == 6) {// 七对爆头翻
				this.UpdateLabelColor(toggles);
    ...
    UpdateTogglesLabel:function (TogglesNode, isResetPos=true) {
		...
		if (this.Toggles["kexuanwanfa"]) {
			if (this.Toggles["kexuanwanfa"][6] && !this.Toggles["kexuanwanfa"][6].getChildByName("checkmark").active) { // 勾选七对爆头翻倍
