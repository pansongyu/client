...
var rcmjChildCreateRoom = cc.Class({
    ...
    UpdateTogglesLabel: function (TogglesNode, isResetPos = true) {
        ...
        if (this.Toggles["zhuama"]) {
            if (this.Toggles["wanfa"][0].getChildByName("checkmark").active) { // 红中159
        ...
        if (this.Toggles['kexuanwanfa']) {
            ...
            // 	财神、边卡吊、带风仅互报互飞可选；
            if (this.Toggles['wanfa'][1].getChildByName('checkmark').active) { // 互报互飞
                let hideIdxs = [2, 3, 4];	// 	财神、边卡吊、带风仅互报互飞可选；
                for (let i = 0; i < hideIdxs.length; i++) {
                    let idx = hideIdxs[i];
    ...
    OnToggleClick: function (event) {
        ...
        if ('kexuanwanfa' == key) {
            // 	互报互飞模式必须勾选带风才能勾选七对翻倍
            if (toggleIndex == 6) {
                let toggle = this.Toggles['kexuanwanfa'][4];
