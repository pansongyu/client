...
cc.Class({
	...
	ShowSpecData: function (setEnd, playerAll, index) {
		...
		let publicCardList = setEnd.publicCardList;
		//倍数
		...
		let roundInfo = player["roundInfo"];
		let opList = roundInfo["opList"];
		let betList = roundInfo["betList"];
	...
	ShowPublicCardList: function (publicCardList, isQiPai) {
		let publiccardsNode = this.node.getChildByName("publiccards");
		for (let i = 0; i < publiccardsNode.children.length; i++) {
			let cardValue = publicCardList[i];
			let cardNode = publiccardsNode.children[i];
			if (cardValue) {
	...
	ShowOpType: function (opList, betList) {
		let opTypeNameDict = {
			"26": "弃牌",//弃牌26
			"25": "allin",//筹码全部下注25
			"23": "加注",//加注23
		...
		let typeNameArr = ["底牌：","翻牌：","转牌：","河牌："];
		let opTypeNode = this.node.getChildByName("optype");
		for (let i = 0; i < opTypeNode.children.length; i++) {
			opTypeNode.children[i].active = false;
		}
		for (let i = 0; i < opList.length; i++) {
			if (opList[i] > 0) {
				opTypeNode.children[i].active = true;
				opTypeNode.children[i].getComponent(cc.Label).string = typeNameArr[i] + opTypeNameDict[opList[i]] + betList[i];
			}
		}
	},
