...
cc.Class({
	...
	ShowData: function () {
		...
		let zhuColor = data["liangZhu"];
		this.ShowZhuCardColor(zhuColor);
		let kouDi = data["kouDi"];
		this.ShowKouDi(kouDi);
		let zhuangBaoFen = data["zhuangBaoFen"];
		let zhuangJianFen = data["zhuangJianFen"];
	...
	ShowZhuCardColor: function (zhuColor) {
		this.flower.spriteFrame = this.huaSeSpriteDict[zhuColor];
	},
	ShowKouDi: function (cardList) {
		for (let i = 0; i < this.koudilayout.children.length; i++) {
			let node = this.koudilayout.children[i];
