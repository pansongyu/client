...
var sssChildCreateRoom = cc.Class({
	...
	CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
		...
		let zhadao = this.GetIdxByKey("zhadao");
		let paixing = this.GetIdxByKey("paixing");
		...
		let languowanfa = [];
		for (let i = 0; i < this.Toggles["languowanfa"].length; i++) {
			if (this.Toggles["languowanfa"][i].getChildByName("checkmark").active) {
				languowanfa.push(i);
			}
		}
		sendPack = {
