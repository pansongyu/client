...
cc.Class({
	...
	InitShowPlayerInfo: function (setEnd) {
		...
		let jiangList = setEnd["jiangList"] || [];
		let zhongJiangList = setEnd["zhong1List"] || [];

		let jin1 = setEnd["jin"] || 0;
		let jin2 = setEnd["jin2"] || 0;
	...
	ShowMaiMaPai: function (cardIDList, zhongList, jin1, jin2) {
		let bg_maima = this.node.getChildByName("mapai").getChildByName("bg_maima");
		if(cardIDList.length==0){
			bg_maima.active=false;
			return;
		}
		bg_maima.active=true;
		let niaopaiNode = bg_maima.getChildByName("niaopai");
		...
		for (let i = 0; i < cardIDList.length; i++) {
			...
			this.ShowImage(node, cardIDList[i]);
			node.active = true;
			node.getChildByName("da").active = (Math.floor(cardIDList[i] / 100) == Math.floor(jin1 / 100) || Math.floor(cardIDList[i] / 100) == Math.floor(jin2 / 100))
			node.color = zhongList.indexOf(cardIDList[i]) == -1 ? cc.color(180, 180, 180) : cc.color(255, 255, 255);
		}
	},
	ShowJiangMaPai: function (cardIDList, zhongList, jin1, jin2) {
		let bg_jiangma = this.node.getChildByName("mapai").getChildByName("bg_jiangma");
		if(cardIDList.length==0){
			bg_jiangma.active=false;
			return;
		}
		bg_jiangma.active=true;
		let niaopaiNode = bg_jiangma.getChildByName("niaopai");
		for(let i=0;i<niaopaiNode.children.length;i++){
			niaopaiNode.children[i].active=false;
		}
		for (let i = 0; i < cardIDList.length; i++) {
	...
	ShowJinNode: function(jin1, jin2){
		let jinNode = this.node.getChildByName("jinNode");
