...
cc.Class({
    ...
    ShowPlayerData: function (setEnd, playerAll, index) {
        ...
        this.UpdatePlayData(this.node, posResultList[index], PlayerInfo, jin1, jin2, maiMaList, index);
    ...
    UpdatePlayData: function (PlayerNode, HuList, PlayerInfo, jin1 = 0, jin2 = 0, maiMaList = [], pos) {
        this.showLabelNum = 1;
        ...
        this.ShowPlayerHuPai(PlayerNode.getChildByName('hupai'), HuList, pos);
    ...
    GetHuTypeDict: function () {
		let huTypeDict = {};
		huTypeDict["ZiMo"] = "摸炸弹（自摸）";
		huTypeDict["ZhuangJia"] = "庄家输赢";
		huTypeDict["PingHu"] = "平胡";

		return huTypeDict;
    },
    // 参照位置，提供位置
	GetProvidePos: function (referencePos, providePos) {
		...
		let offPos = (providePos - referencePos + 2 * playerCount) % playerCount;
		return cfg[playerCount][offPos];
	},
    ShowPlayerHuPai: function (showNode, huInfoAll, pos) {
        let huInfo = false
        ...
        let huInfoMap = huInfo["huInfoMap"];
		let huPosInfo = huInfo["huPosInfo"];
        for (let i = 1; i <= 23; i++) {
            showNode.getChildByName("card" + i).active = false;
        ...
        let count = 0;
        for(let key in huInfoMap){
            count++;
            let node = showNode.getChildByName("card" + count);
            node.cardType = key;
			let providePos = huPosInfo[key];
            //自摸的牌
            if(key < 0){
            ...
            let cardID = Math.floor(Math.abs(key)/100);
            this.ShowImage(node, 'EatCard_Self_', cardID);
            let huTypeStr = ""
            let huTypeMap = huInfoMap[key];
            ...
            let label = node.getChildByName("mapNode").getChildByName("img_tsk").getChildByName("label").getComponent(cc.Label);
            label.string = huTypeStr;
            let lb_provide = this.GetProvidePos(pos, providePos);
			node.getChildByName("lb_provide").getComponent(cc.Label).string = lb_provide;
            node.active = true;
        };
        
    },
    //清除胡牌显示
    ClearHuPaiDis:function(){
        let Players = this.node.parent;
        for (let i = 0; i < Players.children.length; i++) {
            let huPaiNode = Players.children[i].getChildByName("hupai");
            for (let j = 0; j < huPaiNode.children.length; j++) {
                huPaiNode.children[j].getChildByName("mapNode").active = false;
            }
        }
    },
    //所有胡的牌点击事件
    Click_huPai:function(event){
