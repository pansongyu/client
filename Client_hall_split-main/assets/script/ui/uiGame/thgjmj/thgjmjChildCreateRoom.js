...
var thgjmjChildCreateRoom = cc.Class({
    ...
    CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
        let sendPack = {};
        let piaojing = this.GetIdxByKey('piaojing');
        let piaojingcaozuo = this.GetIdxByKey('piaojingcaozuo');
        let gangjingwanfa = this.GetIdxByKey('gangjingwanfa');
        let chaozhuang = this.GetIdxByKey('chaozhuang');
    ...
    UpdateTogglesLabel: function (TogglesNode, isResetPos = true) {
        ...
        if (this.Toggles["kexuanwanfa"]) {
            for (let i = 0; i < this.Toggles["kexuanwanfa"].length; i++) {
                ...
                if (ToggleDesc == "比赛分不能低于0分" || ToggleDesc == "只能赢当前身上分") {
                    if (!this.clubData && !this.unionData) {
