...
var rcwskChildCreateRoom = cc.Class({
    ...
    CreateSendPack: function (renshu, setCount, isSpiltRoomCard) {
        ...
        let qianfenjushuying = this.GetIdxByKey('qianfenjushuying');
                    ...
                    if ('renshu' == curKey || 'fangfei' == curKey || 'jushu' == curKey) {
                        ...
                        if (this.Toggles['jushu']) {
                            ...
                            for (let i = 0; i < curCostData.length; i++) {
								let setCount = curCostData[i].SetCount == 501 ? "千分" : curCostData[i].SetCount;
                                this.Toggles['jushu'][i].getChildByName('label').getComponent(cc.Label).string = setCount + '局';
                            }
                        }
    ...
    OnUpdateTogglesLabel: function (TogglesNode, isResetPos = true) {
        if (this.Toggles["qianfenjushuying"]) {
            if (this.Toggles["jushu"][3] && this.Toggles["jushu"][3].getChildByName("checkmark").active) { // 勾选千分局
                this.Toggles["qianfenjushuying"][0].parent.active = true;
                this.Toggles["wanfa"][0].active = false;

                if (this.Toggles["wanfa"][0].getChildByName("checkmark").active) {	// 摸队为勾选状态
