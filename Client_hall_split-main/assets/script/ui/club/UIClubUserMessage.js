...
cc.Class({
    ...
    OnShow:function(clubId,unionId,unionName, unionSign,pid=0,opClubId=0){
    ...
    InitShowType:function(showType=0){
    ...
    CheckMessageDisplay:function(){
        let messageScrollView = this.node.getChildByName("messageScrollView");
        let ScrollOffset=messageScrollView.getComponent(cc.ScrollView).getScrollOffset();
    ...
    filterShow:function(type){
        //各个类型的消息id
        let typeList={0:[],1:[122,123,124,125,137,138,139,140],2:[1,2,114,115,120,121],3:[119,126,1006,1008],4:[129,130],5:[3],6:[1009,1010,1011,1012]};
        let match=typeList[type];
        let messageScrollView = this.node.getChildByName("messageScrollView");
        let content = messageScrollView.getChildByName("view").getChildByName("content");
        messageScrollView.getComponent(cc.ScrollView).scrollToTop();
        ...
        let demo = this.node.getChildByName("demo");
        for (let i = 0; i < content.children.length; i++) {
            let execType=content.children[i].execType;
            if(match.length>0){
